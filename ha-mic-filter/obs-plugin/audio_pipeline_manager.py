...
class AudioPipelineManager:
    ...
    def load_library(self, library_path: Optional[str] = None) -> bool:
        ...
        try:
            if library_path and os.path.exists(library_path):
                ...
                if system == "Windows":
                    lib_paths = [
                        "./build-win64/obs-mic-filter.dll",
                        "./obs-mic-filter.dll",
                        "obs-mic-filter.dll"
                    ]
                else:
                    lib_paths = [
                        "./build-linux/libobs-mic-filter.so",
    ...
    def remove_filter(self, filter_id: int) -> bool:
        ...
        Returns:
            True if filter removed successfully, False otherwise
        """
        ...
        if filter_id in self.filter_order:
            self.filter_order.remove(filter_id)
        
        # Note: The C library might need a specific function to remove filters
        # For now, we'll disable it by updating with disabled status
        params = obs_filter_params_t()
        ...
        success = result == self.OBS_PIPELINE_SUCCESS
        if success:
            print(f"Removed filter ID: {filter_id}")
        else:
            print(f"Failed to remove filter ID: {filter_id}")
        
        return success
    
    def update_filter(self, filter_id: int, **parameters) -> bool:
        """
        Update filter parameters.
        
        Args:
            filter_id: ID of the filter to update
            **parameters: New parameters for the filter
            
        Returns:
            True if filter updated successfully, False otherwise
        """
        if filter_id not in self.filters:
            print(f"Filter ID {filter_id} not found")
            return False
        ...
        config = self.filters[filter_id]
        config.parameters.update(parameters)
        
        # Create updated parameters structure
        params = obs_filter_params_t()
        params.type = config.filter_type.value
        params.enabled = config.enabled
        
        success = self._set_filter_parameters(params, config.filter_type, config.parameters)
        if not success:
            return False
        ...
        if success:
            ...
            print(f"Failed to update filter ID: {filter_id}")
    ...
    def get_filter_info(self) -> Dict[int, Dict[str, Any]]:
        """
        Get information about all filters in the pipeline.
        
        Returns:
            Dictionary mapping filter IDs to filter information
        """
        info = {}
        for filter_id, config in self.filters.items():
            info[filter_id] = {
                'type': config.filter_type.name,
                'enabled': config.enabled,
                'parameters': config.parameters,
                'order': self.filter_order.index(filter_id) if filter_id in self.filter_order else -1
            }
        return info
    ...
    def create_professional_voice_preset(self) -> List[int]:
        """
        Create a professional voice processing preset.
        
        Returns:
            List of filter IDs in processing order
        """
        filter_ids = []
        
        # 1. RNNoise noise suppression
        filter_id = self.add_filter(
            FilterType.NOISE_SUPPRESS,
            method=NoiseSuppressionMethod.RNNOISE.value,
            suppress_level=-30,
            intensity=0.8
        )
        ...
        filter_id = self.add_filter(
            ...
            release_time=75.0,
            output_gain=2.0,
            detector=ExpanderDetection.RMS.value,
            preset=ExpanderPreset.EXPANDER.value
        )
        if filter_id is not None:
            filter_ids.append(filter_id)
        ...
        print("Professional voice preset created")
        return filter_ids
    
    def print_pipeline_summary(self):
        """Print a summary of the current pipeline configuration"""
        print("=== Audio Pipeline Summary ===")
        print(f"Library loaded: {self.library is not None}")
        print(f"Pipeline created: {self.pipeline is not None}")
        
        if self.pipeline_config:
            print(f"Sample rate: {self.pipeline_config.sample_rate} Hz")
            print(f"Channels: {self.pipeline_config.channels}")
            print(f"Buffer size: {self.pipeline_config.buffer_size_ms} ms")
        
        print(f"Total filters: {len(self.filters)}")
        
        if self.filters:
            print("Filter chain:")
            for i, filter_id in enumerate(self.filter_order, 1):
                if filter_id in self.filters:
                    config = self.filters[filter_id]
                    print(f"  {i}. {config.filter_type.name} (ID: {filter_id}) - Enabled: {config.enabled}")
